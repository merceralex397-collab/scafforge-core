...
def extract_reference_codes(path: Path) -> set[str]:
    ...
    return set(
        re.findall(
            r"\b(?:BOOT|SKILL|MODEL|CYCLE|WFLOW|SESSION|ENV|EXEC)\d{3}\b",
            read_text(path),
        )
    )


def require_reference_sync(
    findings: list[Finding], primary: Path, secondary: Path, *, label: str
) -> None:
...
def require_json_field(
    findings: list[Finding], path: Path, key_path: list[str], expected: object
) -> None:
    if not path.exists():
        add_missing(findings, path)
        return
...
def validate_flow_manifest(findings: list[Finding]) -> None:
    ...
    if not isinstance(manifest, dict):
        findings.append(
            Finding("error", "skills/skill-flow-manifest.json is not a JSON object")
        )
        return
    ...
    if not isinstance(run_types, dict):
        findings.append(
            Finding("error", "skill-flow-manifest.json is missing a run_types object")
        )
        return
    if not isinstance(skills, dict):
        findings.append(
            Finding("error", "skill-flow-manifest.json is missing a skills object")
        )
        return
    ...
    if not isinstance(contract_smells, list):
        findings.append(
            Finding(
                "error", "skill-flow-manifest.json is missing a contract_smells list"
            )
        )
        contract_smells = []
    if not isinstance(flow_graph_sanity, dict):
        findings.append(
            Finding(
                "error",
                "skill-flow-manifest.json is missing a flow_graph_sanity object",
            )
        )
        flow_graph_sanity = {}
    else:
        if flow_graph_sanity.get("single_public_entrypoint") != "scaffold-kickoff":
            findings.append(
                Finding(
                    "error",
                    "flow_graph_sanity.single_public_entrypoint must remain scaffold-kickoff",
                )
            )
        if flow_graph_sanity.get("all_run_types_end_with") != "handoff-brief":
            findings.append(
                Finding(
                    "error",
                    "flow_graph_sanity.all_run_types_end_with must remain handoff-brief",
                )
            )
    ...
    if not isinstance(cycle_smell, dict):
        findings.append(
            Finding(
                "error", "skill-flow-manifest.json must declare contract smell CYCLE001"
            )
        )
    else:
        ...
        if cycle_smell.get("skills") != [
            ...
            findings.append(
                Finding(
                    "error",
                    "CYCLE001 must name project-skill-bootstrap and opencode-team-bootstrap as the affected skills",
                )
            )
        if cycle_smell.get("current_order") != [
    ...
    for run_type, payload in run_types.items():
        if (
            ...
            and payload.get("entrypoint") != "scaffold-kickoff"
        ):
            findings.append(
                Finding(
                    "error", f"{run_type} must use scaffold-kickoff as its entrypoint"
                )
            )
        if isinstance(payload, dict):
            ...
            if (
                ...
                or not sequence
                or sequence[-1] != "handoff-brief"
            ):
                findings.append(
                    Finding("error", f"{run_type} sequence must end with handoff-brief")
                )
    ...
    for skill_name in skills:
        ...
        if not skill_file.exists():
            findings.append(
                Finding(
                    "error",
                    f"Flow manifest references missing skill file: skills/{skill_name}/SKILL.md",
                )
            )
            ...
            for step in sequence:
                ...
                if not skill_name:
                    findings.append(
                        Finding(
                            "error",
                            f"{run_type} contains an invalid sequence step: {step!r}",
                        )
                    )
                    continue
                ...
                if not isinstance(skill_payload, dict):
                    findings.append(
                        Finding(
                            "error",
                            f"{run_type} references unknown skill in its sequence: {skill_name}",
                        )
                    )
                    continue
                if mode:
                    ...
                    if not isinstance(modes, list) or mode not in modes:
                        findings.append(
                            Finding(
                                "error",
                                f"{run_type} references {skill_name}:{mode}, but {skill_name} does not declare that mode",
                            )
                        )
    ...
    if any("scafforge-audit" in str(step) for step in greenfield_sequence):
        findings.append(
            Finding("error", "Greenfield sequence must not include scafforge-audit")
        )
    ...
    if "scafforge-audit:review_and_emit_diagnosis_pack" not in diagnosis_sequence:
        findings.append(
            Finding(
                "error", "diagnosis-review sequence must route through scafforge-audit"
            )
        )
    ...
    for mode in (
        ...
        if mode not in kickoff_modes:
            findings.append(
                Finding("error", f"scaffold-kickoff must expose {mode} mode")
            )
    if "refinement-routing" in kickoff_modes:
        findings.append(
            Finding("error", "scaffold-kickoff must not expose refinement-routing")
        )
    ...
    for mode in ("full-greenfield-pass", "repair-or-regeneration"):
        if mode not in project_skill_modes:
            findings.append(
                Finding("error", f"project-skill-bootstrap must expose {mode} mode")
            )
    ...
    for mode in (
        "full-diagnosis",
        "audit_then_route_repair_if_needed",
        "review_and_emit_diagnosis_pack",
    ):
        if mode not in audit_modes:
            findings.append(
                Finding("error", f"scafforge-audit must expose {mode} mode")
            )
    ...
    for required_skill in ("scafforge-audit", "scafforge-repair", "scafforge-pivot"):
        if required_skill not in skills:
            findings.append(
                Finding(
                    "error",
                    f"skill-flow-manifest.json is missing required skill: {required_skill}",
                )
            )
    ...
    for skill_name, minimum_count in required_output_contracts.items():
        ...
        if not isinstance(outputs, list) or len(outputs) < minimum_count:
            findings.append(
                Finding(
                    "error",
                    f"skill-flow-manifest.json must declare at least {minimum_count} required_outputs for {skill_name}",
                )
            )
...
def validate_core_docs(findings: list[Finding]) -> None:
    ...
    require_contains(
        findings,
        readme,
        "scafforge-audit`, `scafforge-repair`, and `scafforge-pivot` are later lifecycle tools",
    )
    ...
    require_contains(
        findings, competence_contract, "operator confusion is package evidence"
    )
    ...
    require_contains(
        findings, one_shot, "Pivot is outside the initial generation cycle."
    )
    require_contains(
        findings,
        one_shot,
...
def validate_skill_contracts(findings: list[Finding]) -> None:
    ...
    ticket_system_ref = (
        ROOT / "skills" / "ticket-pack-builder" / "references" / "ticket-system.md"
    )
    ...
    pivot_record_script = (
        ...
        / "scafforge-pivot"
        / "scripts"
        / "record_pivot_stage_completion.py"
    )
    pivot_publish_script = (
        ROOT / "skills" / "scafforge-pivot" / "scripts" / "publish_pivot_surfaces.py"
    )
    pivot_apply_script = (
        ROOT / "skills" / "scafforge-pivot" / "scripts" / "apply_pivot_lineage.py"
    )
    pivot_tracking = (
        ROOT / "skills" / "scafforge-pivot" / "scripts" / "pivot_tracking.py"
    )
    pivot_runtime = (
        ROOT / "skills" / "scafforge-pivot" / "scripts" / "generated_tool_runtime.py"
    )
    ...
    require_contains(
        findings,
        scaffold_kickoff,
        "Do not route the initial generation pass into `scafforge-audit` or `scafforge-repair`.",
    )
    require_contains(
        findings,
        scaffold_kickoff,
    ...
    require_contains(
        findings,
        scaffold_kickoff,
        "route to `opencode-team-bootstrap` to add or repair `.opencode/`, then run `project-skill-bootstrap`",
    )
    require_contains(
        findings,
        scaffold_kickoff,
        "let it continue through any required project-specific regeneration or ticket follow-up",
    )
    require_contains(
        findings,
        scaffold_kickoff,
        "same-session immediate-continuation verification gate",
    )
    ...
    require_contains(
        findings,
        scaffold_kickoff,
        "verify_generated_scaffold.py <repo-root> --format both",
    )
    ...
    require_contains(
        findings,
        scaffold_kickoff,
        "explicit ticket acceptance smoke commands as the canonical smoke scope",
    )
    require_contains(
        findings,
        scaffold_kickoff,
        "one legal first move while bootstrap proof is still missing",
    )
    require_contains(findings, scaffold_kickoff, "## Pivot flow")
    require_contains(
        findings,
        scaffold_kickoff,
        "Route midstream feature and design changes through `scafforge-pivot`",
    )
    require_absent(
        findings,
        ROOT / "skills" / "scafforge-audit" / "SKILL.md",
        "greenfield or retrofit flow",
    )
    require_contains(
        findings,
        ROOT / "skills" / "scafforge-audit" / "SKILL.md",
        "retrofit audit step or an explicit diagnosis/review flow",
    )
    require_contains(
        findings,
        ROOT / "skills" / "scafforge-audit" / "SKILL.md",
        "grouped by invariant family in code modules",
    )
    require_contains(
        findings,
        ROOT / "skills" / "scafforge-audit" / "SKILL.md",
        "rule implementation plus regression coverage",
    )
    ...
    require_contains(
        findings,
        repo_factory,
        "No generic placeholder text or template filler may remain in any handoff surface",
    )
    require_contains(
        findings,
        repo_factory,
        "A fresh scaffold must already expose one legal first move while bootstrap proof is missing",
    )
    require_contains(
        findings,
        repo_factory,
        "explicit ticket acceptance smoke commands are canonical smoke scope",
    )
    ...
    require_contains(
        findings,
        project_skill,
        "baseline skill pack and all required synthesized skills in one invocation",
    )
    ...
    require_contains(
        findings,
        project_skill,
        "If the repo is missing `.opencode/skills/`, do not start here.",
    )
    require_contains(
        findings,
        project_skill,
        'a baseline skill that still says "Replace this file..." is invalid',
    )
    require_contains(
        findings,
        project_skill,
        "No generated local skill may retain scaffold placeholder text",
    )
    require_contains(findings, project_skill, "slash commands are human entrypoints")
    require_contains(
        findings,
        project_skill,
        "missing host prerequisites such as `uv`, `pytest`, `rg`, git identity, or service binaries are blockers",
    )
    require_contains(
    ...
    require_contains(
        findings,
        team_bootstrap,
        "In retrofit mode, this skill restores `.opencode/` first",
    )
    require_contains(
        findings,
        team_bootstrap,
        "`project-skill-bootstrap` then creates the repo-local skill pack",
    )
    require_contains(findings, team_bootstrap, "the single visible coordinator")
    require_contains(
        findings,
        team_bootstrap,
        "keep the total agent count conservative unless the canonical brief proves genuinely disjoint domains",
    )
    require_contains(
        findings,
        team_bootstrap,
    ...
    require_contains(
        findings,
        prompt_engineering,
        "Treat these notes as read-only package reference material.",
    )
    require_contains(
        findings,
        prompt_engineering,
        "Do not write back into Scafforge package files during project generation.",
    )
    require_contains(
        findings, prompt_engineering, "Formatting requirements are clear and specific"
    )
    require_contains(
        findings,
        prompt_engineering,
    ...
    require_contains(
        findings,
        ticket_builder,
        "prefer one explicit backticked repo-local smoke command",
    )
    ...
    require_contains(
        findings,
        ticket_builder,
        "literal acceptance command knowingly reaches into sibling-ticket or later-ticket scope",
    )
    ...
    require_contains(
        findings,
        pivot_skill,
        "python3 scripts/record_pivot_stage_completion.py <repo-root>",
    )
    require_contains(
        findings, pivot_skill, "python3 scripts/apply_pivot_lineage.py <repo-root>"
    )
    require_contains(
        findings, pivot_skill, "python3 scripts/publish_pivot_surfaces.py <repo-root>"
    )
    ...
    require_contains(
        findings,
        pivot_record_script,
        "Record completion of a Scafforge pivot downstream stage",
    )
    ...
    require_contains(
        findings, pivot_publish_script, "Republish generated restart surfaces"
    )
    require_contains(
        findings, pivot_publish_script, ".opencode/tools/handoff_publish.ts"
    )
    require_script_help_runs(findings, pivot_publish_script)
    require_contains(
        findings, pivot_apply_script, "Execute explicit pivot ticket lineage actions"
    )
    ...
    require_contains(
        findings,
        handoff,
        "The handoff is a truthful restart surface bounded by current evidence",
    )
    require_contains(
        findings,
        handoff,
    ...
    require_contains(findings, handoff, ".opencode/meta/pivot-state.json")
    require_contains(
        findings,
        handoff,
        "the handoff surfaces reflect the current pivot state truthfully",
    )
    require_contains(
        findings,
        handoff,
        "active pivot state is reflected in `START-HERE.md`, `.opencode/state/context-snapshot.md`, and `.opencode/state/latest-handoff.md`",
    )
    require_contains(
        findings,
        handoff,
...
def validate_template_surfaces(findings: list[Finding]) -> None:
    ...
    environment_bootstrap = (
        TEMPLATE_ROOT / ".opencode" / "tools" / "environment_bootstrap.ts"
    )
    ...
    require_contains(
        findings, TEMPLATE_ROOT / "AGENTS.md", "one active write lane at a time"
    )
    ...
    require_contains(
        findings, TEMPLATE_ROOT / "AGENTS.md", ".opencode/state/latest-handoff.md"
    )
    require_contains(
        findings, TEMPLATE_ROOT / "START-HERE.md", "single-lane-first execution posture"
    )
    require_contains(findings, TEMPLATE_ROOT / "START-HERE.md", "environment_bootstrap")
    require_contains(
        findings, TEMPLATE_ROOT / "START-HERE.md", "bootstrap recovery required"
    )
    require_contains(findings, TEMPLATE_ROOT / "START-HERE.md", "## Generation Status")
    require_contains(
        findings, TEMPLATE_ROOT / "START-HERE.md", "## Post-Generation Audit Status"
    )
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / "docs" / "process" / "workflow.md",
        "default to one active foreground lane at a time",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / "docs" / "process" / "workflow.md",
        "keep one visible team leader by default",
    )
    require_contains(
        findings, TEMPLATE_ROOT / "docs" / "process" / "workflow.md", "`plan_review`"
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / "docs" / "process" / "workflow.md",
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / "docs" / "process" / "workflow.md",
        "run `environment_bootstrap` first, then rerun `ticket_lookup` before any stage change",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / "docs" / "process" / "workflow.md",
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / "docs" / "process" / "workflow.md",
        "open active-ticket work remains the primary foreground lane",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / "docs" / "process" / "workflow.md",
    ...
    require_json_field(
        findings,
        TEMPLATE_ROOT / ".opencode" / "state" / "workflow-state.json",
        ["repair_follow_on", "outcome"],
        "clean",
    )
    require_json_field(
        findings,
        TEMPLATE_ROOT / ".opencode" / "state" / "artifacts" / "registry.json",
        ["version"],
        2,
    )
    require_contains(
    ...
    require_contains(
        findings, TEMPLATE_ROOT / "START-HERE.md", "- split_child_tickets: none"
    )
    require_absent(
        findings, TEMPLATE_ROOT / "START-HERE.md", "repair_follow_on_handoff_allowed"
    )
    require_absent(
        findings,
        TEMPLATE_ROOT / ".opencode" / "state" / "context-snapshot.md",
    ...
    require_absent(
        findings,
        TEMPLATE_ROOT / ".opencode" / "state" / "latest-handoff.md",
        "repair_follow_on_handoff_allowed",
    )
    ...
    require_contains(
        findings, workflow_lib, "export function dependentContinuationAction"
    )
    require_contains(
        findings, workflow_lib, "export function ticketNeedsHistoricalReconciliation"
    )
    require_contains(
        findings, workflow_lib, "export function ticketNeedsTrustRestoration"
    )
    require_contains(
        findings,
        workflow_lib,
    ...
    require_contains(
        findings,
        workflow_lib,
        'workflow.repair_follow_on.outcome === "source_follow_up"',
    )
    require_contains(findings, workflow_lib, "export type PivotState")
    require_contains(findings, workflow_lib, "pivotStatePath")
    require_contains(findings, workflow_lib, "loadPivotState")
    require_contains(findings, workflow_lib, "pivot_in_progress")
    require_contains(findings, workflow_lib, "pivot follow-up required")
    require_contains(findings, workflow_lib, "Continue the pivot follow-on stage")
    require_contains(findings, workflow_lib, "pivot_pending_ticket_lineage_actions")
    require_contains(findings, workflow_lib, "pending_ticket_lineage_actions")
    ...
    require_contains(
        findings, environment_bootstrap, "required bootstrap prerequisites are missing"
    )
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "tools" / "ticket_update.ts",
        '"plan_review"',
    )
    require_contains(
        findings,
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "tools" / "ticket_lookup.ts",
        "historical trust still needs restoration",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "tools" / "ticket_lookup.ts",
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "tools" / "ticket_reconcile.ts",
        "export default tool({",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "tools" / "ticket_reconcile.ts",
        "stale or contradictory source/follow-up linkage",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "tools" / "ticket_reconcile.ts",
    ...
    require_contains(findings, handoff_publish, "loadPivotState")
    require_contains(
        findings, handoff_publish, "refreshRestartSurfaces({ manifest, workflow, pivot"
    )
    require_order(
        findings,
        handoff_publish,
        "const handoffBlocker = await validateHandoffNextAction",
        "await refreshRestartSurfaces",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "tools" / "context_snapshot.ts",
        "loadPivotState",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "tools" / "context_snapshot.ts",
        "renderContextSnapshot(manifest, snapshotState, pivot",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "commands" / "kickoff.md",
        "one active write lane",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "commands" / "kickoff.md",
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "commands" / "resume.md",
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "agents" / "__AGENT_PREFIX__-team-leader.md",
        "when the ticket acceptance criteria already define executable smoke commands",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "agents" / "__AGENT_PREFIX__-team-leader.md",
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "skills" / "ticket-execution" / "SKILL.md",
        "if the ticket acceptance criteria already define executable smoke commands",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "skills" / "ticket-execution" / "SKILL.md",
        "do not convert expected results into PASS evidence",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "skills" / "ticket-execution" / "SKILL.md",
        "slash commands are human entrypoints",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "skills" / "ticket-execution" / "SKILL.md",
        "`repair_follow_on.outcome` is `managed_blocked`",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "skills" / "ticket-execution" / "SKILL.md",
        "ticket_create(source_mode=split_scope)",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "skills" / "ticket-execution" / "SKILL.md",
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "agents" / "__AGENT_PREFIX__-ticket-creator.md",
        "ticket_reconcile: allow",
    )
    require_absent(
        findings,
    ...
    require_contains(
        findings, conformance, '".opencode/skills/model-operating-profile/SKILL.md"'
    )
    ...
    require_contains(
        findings,
        TEMPLATE_ROOT / "tickets" / "README.md",
...
def validate_audit_repair_surfaces(findings: list[Finding]) -> None:
    ...
    require_contains(
        findings, audit_skill / "scripts" / "shared_verifier.py", "tooling_doc_path"
    )
    require_contains(
        findings, audit_skill / "scripts" / "shared_verifier.py", "tickets_readme_path"
    )
    require_contains(
        findings, audit_skill / "scripts" / "shared_verifier.py", "ticket_update_path"
    )
    require_contains(
        findings, audit_skill / "scripts" / "shared_verifier.py", "smoke_test_path"
    )
    require_contains(
        findings, audit_skill / "scripts" / "shared_verifier.py", "handoff_publish_path"
    )
    require_contains(
        findings, audit_skill / "scripts" / "shared_verifier.py", "VERIFY008"
    )
    require_contains(
        findings, audit_skill / "scripts" / "shared_verifier_types.py", "@dataclass"
    )
    require_contains(
        findings, audit_skill / "scripts" / "shared_verifier_types.py", "class Finding"
    )
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
    ...
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
        "contract_surface_audit_context",
    )
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
        "execution_surface_audit_context",
    )
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
        "lifecycle_contract_audit_context",
    )
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
        "repair_cycle_audit_context",
    )
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
        "session_transcript_audit_context",
    )
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
        "restart_surface_audit_context",
    )
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
        "ticket_graph_audit_context",
    )
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
    ...
    require_contains(
        findings, audit_contract_surfaces, "class ContractSurfaceAuditContext"
    )
    require_contains(
        findings, audit_contract_surfaces, "def run_contract_surface_audits("
    )
    require_contains(findings, audit_contract_surfaces, "def audit_status_model(")
    require_contains(
        findings, audit_contract_surfaces, "def audit_model_profile_drift("
    )
    require_contains(
        findings, audit_execution_surfaces, "class ExecutionSurfaceAuditContext"
    )
    require_contains(
        findings, audit_execution_surfaces, "def run_execution_surface_audits("
    )
    require_contains(
    ...
    require_contains(
        findings, audit_lifecycle_contracts, "def run_lifecycle_contract_audits("
    )
    require_contains(
        findings, audit_lifecycle_contracts, "def audit_active_process_verification("
    )
    require_contains(
        findings, audit_lifecycle_contracts, "def audit_ticket_transition_contract("
    )
    ...
    require_contains(
        findings, audit_repair_cycles, "def audit_verification_basis_regression("
    )
    ...
    require_contains(
        findings, audit_session_transcripts, "class SessionTranscriptAuditContext"
    )
    require_contains(
        findings, audit_session_transcripts, "def run_session_transcript_audits("
    )
    require_contains(
        findings, audit_session_transcripts, "def audit_session_chronology("
    )
    require_contains(
        findings, audit_session_transcripts, "def audit_session_operator_trap("
    )
    require_contains(
        findings, audit_restart_surfaces, "class RestartSurfaceAuditContext"
    )
    require_contains(
        findings, audit_restart_surfaces, "def run_restart_surface_audits("
    )
    require_contains(
        findings, audit_restart_surfaces, "def audit_restart_surface_drift("
    )
    require_contains(
        findings, audit_restart_surfaces, "def audit_resume_truth_hierarchy("
    )
    ...
    require_contains(
        findings, audit_ticket_graph, "def audit_historical_reconciliation_deadlock("
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "apply_repo_process_repair.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "apply_repo_process_repair.py",
        'workflow_contract.get("parallel_mode", "sequential")',
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "apply_repo_process_repair.py",
    ...
    require_absent(
        findings,
        repair_skill / "scripts" / "apply_repo_process_repair.py",
        'startswith(("WFLOW", "BOOT", "CYCLE", "ENV"))',
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "apply_repo_process_repair.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "apply_repo_process_repair.py",
        "Run scafforge-audit before relying on this restart narrative for continued development.",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
    ...
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "`stable`, `replace`, `regenerate`, and `ticket_follow_up`",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
    ...
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "hidden legacy `--stage-complete` compatibility shim",
    )
    require_contains(findings, repair_skill / "SKILL.md", "`missing_provenance`")
    require_contains(
        findings, repair_skill / "SKILL.md", "canonical repair follow-on stage catalog"
    )
    ...
    require_contains(
        findings, repair_skill / "SKILL.md", "not part of the current repair cycle"
    )
    require_contains(findings, repair_skill / "SKILL.md", "ticket-pack-builder")
    require_contains(findings, repair_skill / "SKILL.md", "handoff-brief")
    require_contains(
    ...
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "must include a non-empty `completed_by`, a non-empty summary",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "must also match the current repair cycle before Scafforge accepts the recorded completion",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "must invalidate that recorded execution automatically",
    )
    require_contains(
        findings, repair_skill / "SKILL.md", "repair-contract consistency checks"
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "run_managed_repair.py",
        '"required_follow_on_stages"',
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "run_managed_repair.py",
    ...
    require_paths(
        findings,
        [
            repair_skill / "scripts" / "follow_on_tracking.py",
            repair_skill / "scripts" / "record_repair_stage_completion.py",
        ],
    )
    require_contains(
        findings,
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        "must match the current repair cycle before recorded execution can be accepted",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        "requires repair_package_commit provenance",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
    ...
    require_contains(findings, audit_reporting, '"missing_provenance"')
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        '"project-skill-bootstrap"',
    ...
    require_contains(
        findings, repair_skill / "scripts" / "follow_on_tracking.py", '"handoff-brief"'
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
    ...
    require_contains(
        findings, repair_skill / "scripts" / "follow_on_tracking.py", '"agent_team"'
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        "follow_on_stage_history_metadata",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        "OPTIONAL_RECORDABLE_FOLLOW_ON_STAGES",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        "validate_stage_allowed_for_current_cycle",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        "Unknown repair follow-on stage",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        '"project-skill-bootstrap:auto-detected"',
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        '"opencode-team-bootstrap:auto-detected"',
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
        '"agent-prompt-engineering:auto-detected"',
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "follow_on_tracking.py",
    ...
    require_script_help_runs(
        findings, repair_skill / "scripts" / "record_repair_stage_completion.py"
    )
    require_contains(
        findings,
        project_skill_bootstrap_skill := ROOT
        / "skills"
        / "project-skill-bootstrap"
        / "SKILL.md",
        ".opencode/state/artifacts/history/repair/project-skill-bootstrap-completion.md",
    ...
    require_contains(
        findings,
        opencode_team_bootstrap_skill := ROOT
        / "skills"
        / "opencode-team-bootstrap"
        / "SKILL.md",
        ".opencode/state/artifacts/history/repair/opencode-team-bootstrap-completion.md",
    ...
    require_contains(
        findings,
        agent_prompt_engineering_skill := ROOT
        / "skills"
        / "agent-prompt-engineering"
        / "SKILL.md",
        ".opencode/state/artifacts/history/repair/agent-prompt-engineering-completion.md",
    ...
    require_contains(
        findings,
        ticket_pack_builder_skill := ROOT
        / "skills"
        / "ticket-pack-builder"
        / "SKILL.md",
        ".opencode/state/artifacts/history/repair/ticket-pack-builder-completion.md",
    ...
    require_contains(
        findings, ticket_pack_builder_skill, "- completed_stage: ticket-pack-builder"
    )
    require_contains(
        findings,
        ticket_pack_builder_skill,
        "- cycle_id: <cycle_id from .opencode/meta/repair-follow-on-state.json>",
    )
    require_contains(
        findings,
        handoff_brief_skill := ROOT / "skills" / "handoff-brief" / "SKILL.md",
        ".opencode/state/artifacts/history/repair/handoff-brief-completion.md",
    )
    require_contains(findings, handoff_brief_skill, "- completed_stage: handoff-brief")
    require_contains(
        findings,
        handoff_brief_skill,
        "- cycle_id: <cycle_id from .opencode/meta/repair-follow-on-state.json>",
    ...
    require_contains(
        findings, repair_skill / "scripts" / "run_managed_repair.py", '"clean"'
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "run_managed_repair.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "run_managed_repair.py",
        '"non_clean_without_findings"',
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "run_managed_repair.py",
        '"restart_surface_drift_after_repair"',
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "run_managed_repair.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "regenerate_restart_surfaces.py",
        "restart_surface_regeneration_history",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "regenerate_restart_surfaces.py",
    ...
    require_contains(
        findings,
        repair_skill / "scripts" / "regenerate_restart_surfaces.py",
        "process_verification_state(",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "regenerate_restart_surfaces.py",
        "dependent_continuation_action(",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "regenerate_restart_surfaces.py",
        "ticket_needs_historical_reconciliation(",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "regenerate_restart_surfaces.py",
        "ticket_needs_trust_restoration(",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "regenerate_restart_surfaces.py",
        "Current ticket is already closed. Activate dependent ticket",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "regenerate_restart_surfaces.py",
    ...
    require_contains(
        findings,
        audit_lifecycle_contracts,
        "does not expose whether pending_process_verification is immediately clearable",
    )
    ...
    require_contains(
        findings,
        audit_skill / "scripts" / "audit_repo_process.py",
        "extract_transcript_smoke_acceptance_commands",
    )
    require_contains(findings, audit_reporting, '"audit_package_commit"')
    ...
    require_contains(
        findings, repair_skill / "scripts" / "audit_repo_process.py", "module_from_spec"
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "audit_repo_process.py",
    ...
    require_contains(
        findings, repair_skill / "scripts" / "shared_verifier.py", "module_from_spec"
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "shared_verifier.py",
    ...
    require_script_help_runs(
        findings, repair_skill / "scripts" / "run_managed_repair.py"
    )
    require_script_help_runs(
    ...
    require_contains(
        ...
        "Bootstrap deadlock (BOOT001",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Bootstrap command/layout mismatch (BOOT002",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Placeholder local skills (SKILL001",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Model profile drift (MODEL001",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Repeated failed repair cycle (CYCLE001",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Repeated diagnosis churn (CYCLE002",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Verification-basis regression (CYCLE003",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Smoke-test contract drift (WFLOW001",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Handoff overclaim (WFLOW002",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Review-stage ambiguity (WFLOW003",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Transition-contract mismatch (WFLOW004",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Smoke-proof bypass (WFLOW005",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Coordinator workflow overreach (WFLOW006",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Handoff ownership conflict (WFLOW007",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Hidden process-verification state (WFLOW008",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Restart-surface drift (WFLOW010",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Bootstrap-first guidance drift (WFLOW011",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Lease-ownership split (WFLOW012",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Resume truth-hierarchy drift (WFLOW013",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Invocation-log coordinator artifact authorship (WFLOW014",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Broken helper tool surface (WFLOW015",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Smoke-test override execution defect (WFLOW016",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Acceptance-command smoke drift (WFLOW017",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Closed-ticket remediation deadlock (WFLOW018",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Stale ticket-graph contradiction (WFLOW019",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Open-parent split drift (WFLOW020",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Legacy repair-gate leak (WFLOW021",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Closeout lease contradiction (WFLOW022",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Acceptance-scope contamination (WFLOW023",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Historical reconciliation deadlock (WFLOW024",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Thin workflow explainer (SKILL002",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
    ...
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Workflow thrash loop (SESSION002",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Workflow bypass search (SESSION003",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Evidence-free PASS claims (SESSION004",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "Coordinator-authored specialist artifacts (SESSION005",
    )
    require_contains(
        findings,
        audit_skill / "references" / "process-smells.md",
        "No-legal-next-move trap (SESSION006",
    )
    require_contains(
        findings,
    ...
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW015"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW016"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW017"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW018"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW019"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW020"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW021"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW022"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW023"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "WFLOW024"
    )
    require_contains(
        findings, repair_skill / "references" / "repair-playbook.md", "SESSION006"
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
    ...
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW008`: keep `pending_process_verification` visible in restart surfaces and `ticket_lookup.transition_guidance`",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW010`: regenerate `START-HERE.md`, `.opencode/state/context-snapshot.md`, and `.opencode/state/latest-handoff.md`",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "lib" / "workflow.ts",
    ...
    require_contains(
        findings, TEMPLATE_ROOT / ".opencode" / "lib" / "workflow.ts", "clearable_now"
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "lib" / "workflow.ts",
        "Cannot publish clean-state or readiness claims while",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "plugins" / "stage-gate-enforcer.ts",
        "isWorkflowProcessVerificationClearOnly",
    )
    require_contains(
        findings,
        TEMPLATE_ROOT / ".opencode" / "plugins" / "stage-gate-enforcer.ts",
    ...
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW011`: refresh `ticket_lookup`, the team-leader prompt, and `ticket-execution` together",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW012`: refresh workflow docs, resume-facing commands, and worker prompts together",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW013`: refresh `/resume`, repo guidance docs, and restart surfaces together",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW014`: treat coordinator-authored specialist artifacts from `.opencode/state/invocation-log.jsonl` as suspect evidence",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW015`: keep shared workflow helpers private to imports",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW016`: refresh `.opencode/tools/smoke_test.ts` so `command_override` accepts tokenized argv, one shell-style command string, or multiple shell-style command strings",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW017`: refresh `.opencode/tools/smoke_test.ts`, `ticket-execution`, and the team-leader prompt together",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW018`: refresh `.opencode/plugins/stage-gate-enforcer.ts`, `ticket_create`, and `issue_intake` together",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW019`: add or repair `ticket_reconcile`",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW020`: add first-class `split_scope` support",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW021`: keep legacy `handoff_allowed` parsing internal only",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW022`: keep closeout publication outside the ordinary open-ticket lease contract",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW023`: tighten `ticket-pack-builder`, ticket docs, and closeout guidance together",
    )
    require_contains(
        findings,
        audit_skill / "references" / "repair-playbook.md",
        "`WFLOW024`: repair `ticket_reconcile`, historical evidence routing, and closeout publication together",
    )
    require_contains(
        findings,
        audit_skill / "SKILL.md",
    ...
    require_contains(
        findings,
        audit_skill / "SKILL.md",
        "whether failed bootstrap artifacts show command traces that contradict the repo's declared dependency layout",
    )
    require_contains(
        findings,
        audit_skill / "SKILL.md",
        "Do not collapse repeated incompatible bootstrap command traces into a generic `ENV002` rerun recommendation",
    )
    require_contains(
        findings,
        audit_skill / "SKILL.md",
    ...
    require_contains(
        findings,
        audit_skill / "SKILL.md",
        "operator spending time figuring out how to move forward on basic workflow",
    )
    require_contains(findings, audit_skill / "SKILL.md", "one clear legal next move")
    require_contains(
        findings,
        audit_skill / "SKILL.md",
        "zero-finding verification audit as proof of repair if the earlier transcript-backed causal basis was dropped",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "If the repair basis includes a transcript-backed `smoke_test` override failure, treat that as workflow-surface drift",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
    ...
    require_contains(
        findings,
        audit_skill / "SKILL.md",
        "Do not tell the user to go straight from report generation to repair",
    )
    require_contains(
        findings,
        audit_skill / "SKILL.md",
    ...
    require_contains(
        findings, audit_skill / "SKILL.md", "previous audit-to-repair cycle failed"
    )
    require_contains(findings, audit_skill / "SKILL.md", "--supporting-log <path>")
    require_contains(
        findings, audit_skill / "SKILL.md", "--diagnosis-output-dir <writable-path>"
    )
    require_contains(
        findings, audit_skill / "SKILL.md", "--diagnosis-kind post_package_revalidation"
    )
    ...
    require_contains(
        findings,
        audit_skill / "SKILL.md",
        "historical session truth and current repo truth",
    )
    ...
    require_contains(
        findings, audit_skill / "SKILL.md", "coordinator-authored specialist artifacts"
    )
    require_contains(
        findings, audit_skill / "SKILL.md", ".opencode/state/invocation-log.jsonl"
    )
    require_absent(findings, audit_skill / "SKILL.md", "read-only skill")
    require_absent(
        findings, audit_skill / "agents" / "openai.yaml", "read-only diagnosis"
    )
    require_contains(
        findings, repair_skill / "SKILL.md", "run `../project-skill-bootstrap/SKILL.md`"
    )
    require_contains(
        findings, repair_skill / "SKILL.md", "run `../opencode-team-bootstrap/SKILL.md`"
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "run `../agent-prompt-engineering/SKILL.md`",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
    ...
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "repeat repair after a prior audit->repair attempt already failed",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "Treat deprecated package-managed defaults as repair drift, not protected intent",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
    ...
    require_contains(
        findings, repair_skill / "SKILL.md", "coordinator-authored PASS artifacts"
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "verification failed and then later recovered with real command evidence",
    )
    require_contains(
        findings, repair_skill / "SKILL.md", "--diagnosis-output-dir <writable-path>"
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "--repair-basis-diagnosis <diagnosis-dir-or-manifest>",
    )
    ...
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "automatically carry forward transcript-backed diagnosis evidence",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "current-state cleanliness from causal-regression verification",
    )
    require_contains(
        findings, repair_skill / "SKILL.md", ".opencode/state/latest-handoff.md"
    )
    require_contains(
        findings, repair_skill / "SKILL.md", "ready for continued development"
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
        "If `BOOT001` or `BOOT002` was part of the repair basis",
    )
    require_contains(
        findings,
        repair_skill / "SKILL.md",
    ...
    require_absent(
        findings,
        repair_skill / "SKILL.md",
        "Pass `--supporting-log <path>` when the repair basis depended on transcript evidence",
    )
    require_contains(
        findings,
        repair_skill / "scripts" / "apply_repo_process_repair.py",
...
def validate_no_hidden_defaults(findings: list[Finding]) -> None:
    ...
    for path in ROOT.rglob("*"):
        ...
        if "devdocs" in path.parts:
            continue
        if "session-analysis" in path.parts:
            continue
