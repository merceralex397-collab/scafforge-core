...
The `blender-mcp` hardening work landed through PR `#3`, but the Scafforge-side lesson was broader than just better adjacent-repo docs. The trust boundary only became solid once the adjacent repo emitted machine-readable authority:
...
- plan `14` is merged in `blender-mcp`
