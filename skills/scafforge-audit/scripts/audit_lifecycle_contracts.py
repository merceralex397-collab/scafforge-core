...
class LifecycleContractAuditContext:
    ...
    tickets_needing_process_verification: Callable[[dict[str, Any], dict[str, Any]], list[dict[str, Any]]]
    parse_start_here_state: Callable[[str], dict[str, Any]]
    normalize_restart_surface_value: Callable[[Any], Any]


def audit_review_stage_ambiguity(root: Path, findings: list[Finding], ctx: LifecycleContractAuditContext) -> None:
...
def audit_ticket_transition_contract(root: Path, findings: list[Finding], ctx: LifecycleContractAuditContext) -> None:
    ticket_update = root / ".opencode" / "tools" / "ticket_update.ts"
    workflow_tool = root / ".opencode" / "lib" / "workflow.ts"
    ...
    if 'ticket.status !== "plan_review"' in ticket_text:
        evidence.append(f"{ctx.normalize_path(ticket_update, root)} still gates implementation on `ticket.status` instead of lifecycle `ticket.stage`.")
    if 'ticket.status !== "plan_review"' in stage_gate_text:
        evidence.append(f"{ctx.normalize_path(stage_gate, root)} still preflights implementation against `ticket.status` instead of lifecycle `ticket.stage`.")
    if "resolveRequestedTicketProgress" not in ticket_text or "validateLifecycleStageStatus" not in ticket_text:
        evidence.append(f"{ctx.normalize_path(ticket_update, root)} does not resolve and validate the requested stage/status pair before mutation.")
    if "LIFECYCLE_STAGES" not in workflow_text or "Unsupported ticket stage:" not in workflow_text:
        evidence.append(f"{ctx.normalize_path(workflow_tool, root)} does not expose an explicit allowed-stage contract with an unsupported-stage error.")
    if "resolveRequestedTicketProgress" not in stage_gate_text or "validateLifecycleStageStatus" not in stage_gate_text:
        evidence.append(f"{ctx.normalize_path(stage_gate, root)} does not validate stage/status requests before tool execution.")
...
def audit_reverification_deadlock(root: Path, findings: list[Finding], ctx: LifecycleContractAuditContext) -> None:
    stage_gate = root / ".opencode" / "plugins" / "stage-gate-enforcer.ts"
    ...
    if "ensureTargetTicketWriteLease(ticketId)" in ticket_reverify_block:
        evidence.append(f"{ctx.normalize_path(stage_gate, root)} still requires a normal write lease before `ticket_reverify` can run.")
    if "cannot be claimed because it is already closed" in ticket_claim_text:
        evidence.append(f"{ctx.normalize_path(ticket_claim, root)} still forbids claiming closed tickets.")
    if (
        ...
        evidence.append(f"{ctx.normalize_path(ticket_lookup, root)} still presents closed tickets as terminal even when process verification may still be pending.")
    ...
    ctx.add_finding(
        ...
        Finding(
            ...
            files=[
                ...
                *([ctx.normalize_path(ticket_lookup, root)] if ticket_lookup.exists() else []),
            ],
...
def audit_smoke_test_artifact_bypass(root: Path, findings: list[Finding], ctx: LifecycleContractAuditContext) -> None:
    ...
    if re.search(r'description:\s*"[^"]*smoke-test[^"]*artifact', artifact_write_text):
        evidence.append(f"{ctx.normalize_path(artifact_write, root)} still advertises smoke-test stages as generic artifact_write targets.")
    if re.search(r'description:\s*"[^"]*smoke-test[^"]*artifact', artifact_register_text):
        evidence.append(f"{ctx.normalize_path(artifact_register, root)} still advertises smoke-test stages as generic artifact_register targets.")
    if "RESERVED_ARTIFACT_STAGES" not in stage_gate_text:
        evidence.append(f"{ctx.normalize_path(stage_gate, root)} does not reserve smoke-test artifacts for their owning tool.")
    if "Generic artifact_write is not allowed for that stage." not in stage_gate_text:
        evidence.append(f"{ctx.normalize_path(stage_gate, root)} does not block generic artifact_write for smoke-test stages.")
    if "Generic artifact_register is not allowed for that stage." not in stage_gate_text:
        evidence.append(f"{ctx.normalize_path(stage_gate, root)} does not block generic artifact_register for smoke-test stages.")
    if "Do not fabricate a PASS artifact through generic artifact tools." not in ticket_lookup_text:
        evidence.append(f"{ctx.normalize_path(ticket_lookup, root)} does not warn that smoke-test PASS proof must come from `smoke_test` rather than generic artifact tools.")
...
def audit_handoff_artifact_ownership_conflict(root: Path, findings: list[Finding], ctx: LifecycleContractAuditContext) -> None:
    stage_gate = root / ".opencode" / "plugins" / "stage-gate-enforcer.ts"
    ...
    ctx.add_finding(
        ...
        Finding(
            ...
            files=[ctx.normalize_path(stage_gate, root), ctx.normalize_path(docs_handoff, root)],
...
def audit_active_process_verification(root: Path, findings: list[Finding], ctx: LifecycleContractAuditContext) -> None:
    ...
    evidence: list[str] = []
    files = [ctx.normalize_path(workflow_path, root), ctx.normalize_path(manifest_path, root)]
    ...
    for surface_path in (start_here_path, latest_handoff_path):
        ...
        if ctx.normalize_restart_surface_value(surface.get("pending_process_verification")) != "true":
            evidence.append(f"{label} does not show pending_process_verification = true while canonical workflow state does.")
        if ctx.normalize_restart_surface_value(surface.get("handoff_status")) == "ready for continued development":
    ...
    if ticket_lookup_path.exists():
        ...
        if "ticket_reverify" not in lookup_text or "process_verification" not in lookup_text:
            evidence.append(f"{ctx.normalize_path(ticket_lookup_path, root)} does not expose the backlog-verifier or ticket_reverify path for pending process verification.")
        if not affected and "clearable_now" not in lookup_text:
            evidence.append(f"{ctx.normalize_path(ticket_lookup_path, root)} does not expose whether pending_process_verification is immediately clearable when the affected done-ticket set is empty.")
    else:
        files.append(ctx.normalize_path(ticket_lookup_path, root))
        evidence.append(f"Missing ticket lookup tool while process verification remains pending: {ctx.normalize_path(ticket_lookup_path, root)}.")

    if not affected:
        files.append(ctx.normalize_path(ticket_update_path, root))
        if not ticket_update_path.exists():
            evidence.append(f"Missing ticket_update tool while pending_process_verification still needs a clear path: {ctx.normalize_path(ticket_update_path, root)}.")
        ...
        if stage_gate_path.exists():
            ...
            if "isWorkflowProcessVerificationClearOnly" not in stage_gate_text or "processVerification.clearable_now" not in stage_gate_text:
                evidence.append(f"{ctx.normalize_path(stage_gate_path, root)} still appears to require a normal ticket write lease even when pending_process_verification is clearable now.")
        else:
            evidence.append(f"Missing stage-gate enforcer while pending_process_verification still needs a legal clear path: {ctx.normalize_path(stage_gate_path, root)}.")
...
def run_lifecycle_contract_audits(root: Path, findings: list[Finding], ctx: LifecycleContractAuditContext) -> None:
